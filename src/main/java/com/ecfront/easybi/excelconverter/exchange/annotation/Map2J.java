...
public @interface Map2J {
     ...
     * <p> 示例：父类(Parent)中有一成员变量(List<Child> children)定义了范围为Mapping("A0:B2")，子类(Child)有一成员变量(name)定义了某一行Mapping("A")，在转换时这一变量(name)最终会被解析为A0:A2。</p>
     */
    String value();


    /**
     * 显式指定成员变量的class，如成员变量为泛型容器（List<SomeClass> vals）或明确的类（SomeClass val）时系统可以自行解析，无需指定此值。
     */
